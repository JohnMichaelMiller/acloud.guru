...
securityGroupId=$(aws ec2 create-security-group --description "This ALB SG allows traffic only on port 80 and port 22" --group-name "ALBWebDMZ" | jq -r .GroupId)
...
aws ec2 authorize-security-group-ingress --group-id $securityGroupId --protocol tcp --port 22 --cidr $myCidr
aws ec2 authorize-security-group-ingress --group-id $securityGroupId --protocol tcp --port 80 --cidr $allCidr
aws ec2 describe-security-groups  --group-ids $securityGroupId | jq -r ".SecurityGroups[].GroupId"
...
availabilityZone=$(echo "$availabilityZones" | jq -r ".AvailabilityZones[0].ZoneName")
...
instances=$(aws ec2 run-instances --image-id $ami --count 2 --key-name $keyName --security-group-ids $securityGroupId --associate-public-ip-address --instance-type $instanceType --user-data "$userData" --placement $placement)

instanceId1=$(echo $instances | jq -r '.Instances | .[0] | .InstanceId')
instanceId2=$(echo $instances | jq -r '.Instances | .[1] | .InstanceId')

echo $instanceId1, $instanceId2

aws ec2 wait instance-running --instance-ids $instanceId1 $instanceId2

aws ec2 describe-instances --instance-ids $instanceId1 $instanceId2 | jq -r ".Reservations[].Instances[].State"

publicDNS1=$(aws ec2 describe-instances --instance-ids $instanceId1 $instanceId2 | jq -r ".Reservations[].Instances[0].NetworkInterfaces[].Association.PublicDnsName")
publicDNS2=$(aws ec2 describe-instances --instance-ids $instanceId1 $instanceId2 | jq -r ".Reservations[].Instances[0].NetworkInterfaces[].Association.PublicDnsName")
...
loadBalancer=$(aws elbv2 create-load-balancer --name my-application-load-balancer  --subnets $subnet1 $subnet2 --security-groups $securityGroupId)

loadBalancerArn=$(echo "$loadBalancer" | jq -r ".LoadBalancers[].LoadBalancerArn")
echo $loadBalancerArn
loadBalancerDns=$(echo "$loadBalancer" | jq -r ".LoadBalancers[].DNSName")
...
listenerArn=$(aws elbv2 create-listener --load-balancer-arn $loadBalancerArn --protocol HTTP --port 80 --default-actions Type=forward,TargetGroupArn=$targetGroupArn  | jq -r ".Listeners[].ListenerArn")
echo "$listenerArn"
...
aws ec2 terminate-instances --instance-ids $instanceId1 $instanceId2 | jq -r '.TerminatingInstances | .[] | .InstanceId, .PreviousState, .CurrentState'
