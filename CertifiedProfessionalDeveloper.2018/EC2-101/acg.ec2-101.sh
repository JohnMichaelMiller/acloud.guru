...
aws ec2 authorize-security-group-ingress --group-id $securityGroupId --protocol tcp --port 80 --cidr $myCidr
...
instanceId=$(aws ec2 run-instances --image-id $ami --count 1 --key-name $keyName --security-group-ids $securityGroupId --associate-public-ip-address --instance-type $instanceType | jq -r '.Instances | .[0] | .InstanceId')
# Assertion
echo $instanceId
...
aws ec2 describe-instances --instance-ids $instanceId | jq -r '.Reservations | .[] | .Instances | .[] | .InstanceId,.State'


# logon to instance
publicDNS=$(aws ec2 describe-instances --instance-ids $instanceId | jq -r '.Reservations | .[] | .Instances | .[] | .NetworkInterfaces | .[] | .Association.PublicDnsName')
# Assertion
echo $publicDNS
...
aws ec2 terminate-instances --instance-ids $instanceId | jq -r '.TerminatingInstances | .[] | .InstanceId, .PreviousState, .CurrentState'
